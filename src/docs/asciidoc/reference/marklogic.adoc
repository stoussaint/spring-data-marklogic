...
- If the entity as no id sets, a new identifier is generated by default as a String representation of UUID.randomUUID(). And the content is inserted
- If the entity as an id set but no existing content in database with this id, then content will be inserted
...
- entityClass : The entity tyoe (Class object)
...
----
include::{spring-data-marklogic-source}/org/springframework/data/marklogic/core/MarklogicOperations.java[lines=170;182;193;200;212;223]
----